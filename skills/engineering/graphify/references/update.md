...
- [Incremental update](#for---update-incremental-re-extraction)
- [Cluster-only](#for---cluster-only)
...
If `code_only` is False (any changed file is a doc/paper/image/video): **first, if any changed file is in `new_files['video']`, return to `SKILL.md`, select the transcription branch for those files, then rewrite `.graphify_detect.json` to move the resulting transcript paths into `files['document']` and drop `files['video']`** — otherwise raw `.mp4/.mp3` paths are fed to semantic extraction as unreadable media (#1392). Then run the full Steps 3A–3C pipeline as normal.
...
Before the merge step, materialize and execute this payload through the runtime contract:

```python
from shutil import copyfile

copyfile('graphify-out/graph.json', 'graphify-out/.graphify_old.json')
```
Afterward, delete `.graphify_old.json` with the active shell's targeted cleanup block from the runtime reference.
