...
```bash
graphify_python -m graphify export wiki
```
...
```bash
graphify_python -m graphify export neo4j
```

**If `--neo4j-push <uri>`** - push directly to a running Neo4j instance. Ask the user for credentials if not provided:

```bash
graphify_python -m graphify export neo4j --push bolt://localhost:7687 --user neo4j --password PASSWORD
```
...
```bash
graphify_python -m graphify export falkordb
```
...
```bash
graphify_python -m graphify export falkordb --push falkordb://localhost:6379
```
...
```bash
graphify_python -m graphify export svg
```

### Step 7c - GraphML export (only if --graphml flag)

```bash
graphify_python -m graphify export graphml
```
...
To configure in Claude Desktop, add to `claude_desktop_config.json`. Resolve the absolute interpreter path by materializing and executing this payload through the runtime contract, then use its output as `command`:
...
```json
{
  "mcpServers": {
    "graphify": {
      "command": "<absolute interpreter path from graphify_python>",
...
```bash
graphify_python -m graphify benchmark
```
