...
- [Query expansion](#step-0--constrained-query-expansion-required-before-traversal)
- [Traversal](#step-1--traversal)
- [Path](#for-graphify-path)
- [Explain](#for-graphify-explain)
...
```
After reading the vocabulary and choosing the selection, delete `graphify-out/.vocab.txt`; it is temporary query state. If the list is empty, say so plainly and stop — do not proceed to traversal.
...
```bash
graphify_python -m graphify query "QUESTION"
# or: graphify_python -m graphify query "QUESTION" --dfs --budget 3000
```
...
Only when the user explicitly asks to persist or learn from the traversal, save the answer for future queries. Include the expanded tokens inside the `--answer` text (e.g. `"Expanded from original query via vocab: [tokens]. Then traversed..."`) so the next `--update` can extract the expansion history as a graph node:

```bash
graphify_python -m graphify save-result --question "ORIGINAL_QUESTION" --answer "ANSWER" --type query --nodes NODE1 NODE2
```

Replace `ORIGINAL_QUESTION` with the user's verbatim question, `ANSWER` with your full answer text (containing the expanded-token trace), `NODE1 NODE2` with the list of node labels you cited. This closes the feedback loop: the next `--update` will extract this Q&A as a node in the graph.

**Work memory (opt-in).** When the user asked to persist the traversal, add an `--outcome` so future sessions learn from it — append `--outcome useful|dead_end|corrected` to the `save-result` command (and `--correction "the right answer"` when correcting):
...
At the start of graph work, read `graphify-out/reflections/LESSONS.md` if it already exists. It lists **preferred sources** (start there), **known dead ends** (skip them), and prior **corrections**. Run `graphify_python -m graphify reflect --if-stale` only when the user explicitly asks to refresh or use persistent work memory; it may update derived files.
...
```bash
graphify_python -m graphify path "NODE_A" "NODE_B"
```
...
Only when the user explicitly asks to persist or learn from the traversal, save the explanation:

```bash
graphify_python -m graphify save-result --question "Path from NODE_A to NODE_B" --answer "ANSWER" --type path_query --nodes NODE_A NODE_B
```
...
```bash
graphify_python -m graphify explain "NODE_NAME"
```
...
```bash
graphify_python -m graphify save-result --question "Explain NODE_NAME" --answer "ANSWER" --type explain --nodes NODE_NAME
```
