...
Do not build a custom dependency analyzer when an established tool can express the invariant. Do not promote an audit or exploration tool into a durable dependency merely because it can display the current graph.
...
2. Where safe and practical, introduce one reversible representative violation at the governed boundary. For a forbidden relationship, add it. For a required relationship, remove or bypass it. For a cycle rule, introduce the smallest representative cycle.
...
5. Run the repository's broader local verification path and any proportionate configuration or CI validation.

Prefer an existing fixture or isolated temporary worktree for destructive proof. Never leave the representative violation committed. If a failure proof is unsafe or impractical, state the exact reason and the strongest proof performed instead.
...
Search for textual instructions that previously carried the exact enforced rule. Remove only prose that is fully superseded by the mechanism and whose durable authority remains discoverable from the enforcement point.

Keep decision rationale, intentional exceptions, user-facing explanation, and any instruction that still requires judgment. Do not delete ambiguous or broader guidance merely because some of it overlaps the new check.
