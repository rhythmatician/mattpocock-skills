...
def build_exposure_matrix(sessions: list[Session], stats: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    for session in sessions:
        for name in sorted(session.exposed_tools | session.tool_set):
            ...
            matrix.append({
                ...
                "exposure_source": session.exposure_source if name in session.exposed_tools else "not_observed",
            })
    return matrix


def exposure_matrix_summary(sessions: list[Session], stats: dict[str, Any]) -> dict[str, int]:
    return {
