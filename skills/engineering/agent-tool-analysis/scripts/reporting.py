...
def _append_table(lines: list[str], headers: str, separator: str, rows: Iterable[str]) -> None:
...
def render_markdown(report: dict[str, Any]) -> str:
    ...
    for key in (
        "sessions_total", "sessions_with_calls", "sessions_with_direct_exposure",
        "sessions_with_calls_and_exposure", "sessions_with_calls_without_exposure",
    ...
    lines.extend([
        ...
        "- Sources: " + ", ".join(f"{name}={count}" for name, count in report["corpus"]["sources"].items()),
        "",
    ...
    for variant in report["architecture_variants"]:
        ...
        for model in EXPOSURE_MODELS:
            lines.extend([f"#### Exposure model: `{model}`", "", EXPOSURE_MODEL_DESCRIPTIONS[model], ""])
            ...
            _append_table(
                ...
                (
                    f"| {label} | " + " | ".join(
                        "unavailable" if metrics[scenario][key] is None else (
                            f"{metrics[scenario][key]:.1%}" if key == "relative_token_reduction" else f"{metrics[scenario][key]:.1f}"
                        ) for scenario in COST_SCENARIOS
                    ) + " |"
                    for key, label in (("baseline_tokens_per_session", "Baseline tokens/session"), ("proposed_tokens_per_session", "Proposed tokens/session"), ("absolute_token_reduction_per_session", "Absolute reduction"), ("relative_token_reduction", "Relative reduction"), ("specialist_activation_rate", "Specialist activation rate"))
                ),
            )
    ...
    if github is None:
        ...
        lines.extend([
            "This is diagnostic sensitivity analysis, not reconstructed telemetry. " + github["assumption"], "",
    ...
    if subset is None:
        ...
        for row in subset["pareto_frontier"]:
            lines.append(f"- {format_tools(row['tools'])}: break-even {_number(row['break_even_exposure_rate_mid'], '%')}, definition {_number(row['definition_tokens_mid'])}, activation {row['activation_rate']:.1%}")
        lines.append("")
    ...
    lines.extend(["## Provider availability reconstruction", "", "Availability and mappings below come only from explicit dynamic-tools groups; runtime calls never establish provider availability.", ""])
    _append_table(
        lines,
        "| Provider | Groups observed | Sessions | Advertised tools |",
        "|---|---:|---:|---|",
        (f"| `{row['provider']}` | {row['group_count']} | {row['session_count']} | {format_tools(row['tools_advertised'])} |" for row in provider["provider_groups_observed"]),
    )
    ...
    if not report["dependency_warnings"]:
        ...
        for warning in report["dependency_warnings"]:
            lines.append(f"- {warning['candidate_id']}: {warning['missing_dependencies']}")
    lines.extend(["", "## Strongest tool relationships", ""])
    _append_table(
        lines,
        "| Tool A | Tool B | Affinity | Jaccard | Overlap | Adjacent calls |",
        "|---|---|---:|---:|---:|---:|",
        (f"| `{pair['tool_a']}` | `{pair['tool_b']}` | {pair['affinity']:.3f} | {pair['jaccard']:.3f} | {pair['overlap']:.3f} | {pair['adjacency_count']} |" for pair in report["strongest_pairs"][:30]),
    )
...
def print_summary(report: dict[str, Any], json_path: Any, markdown_path: Any) -> None:
    ...
    print(f"Expected known-token savings/session: {overhead['expected_known_tokens_saved_per_session']:.1f} ({savings:.1%})" if savings is not None else "Expected known-token savings/session: unavailable")
