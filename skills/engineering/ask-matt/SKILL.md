...
- **`/agent-tool-analysis`** — run the single telemetry optimizer when you need to remove dead tools or decide whether specialist agents beat one pruned agent. It produces a recommendation; replay is an optional validation escape hatch, not part of the normal path.
